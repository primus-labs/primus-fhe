...
use algebra::{
    derive::{Field, Prime, Random}, Field, ListOfProductsOfPolynomials, Polynomial_, Random
};
...
fn evaluate_mv_at_a_point() {
    ...
    products.push(field_vec!(FF; 1, 2, 3, 4));
    products.push(field_vec!(FF; 5, 4, 2, 9));
    let products: Vec<Rc<DenseMultilinearExtension<FF>>> = products
                .into_iter()
                .map(|x| Rc::new(DenseMultilinearExtension::from_evaluations_vec(nv, x)))
                .collect();
    let coeff = FF::new(4);
    poly.add_product(products.into_iter(), coeff);
