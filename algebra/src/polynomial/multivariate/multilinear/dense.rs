...
use crate::{
    ...
    polynomial::multivariate::PolynomialTrait,
};
...
impl<F: Field + Random> PolynomialTrait<F> for DenseMultilinearExtension<F> {
    type Point = Vec<F>;
    ...
    for DenseMultilinearExtension<F>
{
    fn add_assign(&mut self, (f, rhs): (F, &'a DenseMultilinearExtension<F>)) {
        let rhs = Self {
            ...
            evaluations: rhs.iter().map(|&x| f.mul(x)).collect(),
        };
        *self = &*self + &rhs;
    }
}
