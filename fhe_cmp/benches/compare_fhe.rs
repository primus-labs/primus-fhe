...
use fhe_cmp::{FheCompare, Encryptor, KeyGen, LVL2PARAM_128_BITS_PARAMETERS};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("greater_than", |b| {
        b.iter(|| cmp.greater_than::<M>(black_box(&c1), black_box(&c2),black_box(PLAIN_MODULUS_BITS)))
    });

    c.bench_function("greater_than_equal", |b| {
        b.iter(|| cmp.greater_than_equal::<M>(black_box(&c1), black_box(&c2),black_box(PLAIN_MODULUS_BITS)))
    });

    c.bench_function("equal", |b| {
        b.iter(|| cmp.equal::<M>(black_box(&c1), black_box(&c2),black_box(PLAIN_MODULUS_BITS)))
    });
}
