...
fn main() {
    ...
    for i in 0..50 {
        ...
        if x < y {
            if lt_value != 1 || eq_value != param.t() - 1 || gt_value != param.t() - 1 {
                println!("          error1!:{lt_value}:{eq_value}:{gt_value}");
            }
        } else if x == y {
            if lt_value != param.t() - 1 || eq_value != 1 || gt_value != param.t() - 1 {
                println!("          error2!:{lt_value}:{eq_value}:{gt_value}");
            }
        } else {
            if lt_value != param.t() - 1 || eq_value != param.t() - 1 || gt_value != 1 {
