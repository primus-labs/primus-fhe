use algebra::{
    ...
    Basis, DecomposableField, Field, NTTPolynomial, Polynomial,
};
use fhe_cmp::cmp;
use fhe_core::{
    BlindRotationType, ConstParameters, ModulusSwitchRoundMethod, Parameters, RingSecretKeyType,
    SecretKeyPack, SecretKeyType, StepsAfterBR,
};
...
fn main() {
    ...
    let num2 = RGSW::generate_random_one_sample(&mut rng, basis, sampler, sk.ntt_ring_secret_key());
    ...
    */
    let in1 = cmp::rlwe_turn(num1, 4);
    let in2 = cmp::rgsw_turn(num2, 2);
    let out = cmp::greater_hcmp(&in1, &in2);
