use fhe_cmp::comparison::{self, decrypt, encrypt, DELTA, HALF_DELTA,Compare};
...
for _ in 0..50{
    ...
    let out1 = Compare::greater_arbhcmp(&rotationkey,&value1, &value2, DELTA, HALF_DELTA, poly_length);
    let out2 = Compare::less_arbhcmp(&rotationkey, &value1, &value2, DELTA, HALF_DELTA, poly_length);
    ...
    if x > y {
        if decoded_value1 != 1 || decoded_value2 != param.t() - 1 || decoded_value3 != param.t() - 1{
            println!("error1");
        }
    }else if x < y {
        if decoded_value1 != param.t() - 1 || decoded_value2 != 1 || decoded_value3 != param.t() - 1{
            println!("error2");
        }
    }else{
        if decoded_value1 != param.t() - 1 || decoded_value2 != param.t() - 1 || decoded_value3 != 1{
            println!("error3");
        }
    }
}
