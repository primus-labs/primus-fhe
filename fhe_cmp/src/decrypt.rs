...
use fhe_core::{LweCiphertext,encode,decode,LweParameters, LweSecretKey};
...
impl<C: UnsignedInteger, LweModulus: RingReduce<C>> Decryptor<C, LweModulus>{
    ...
    {
        self.lwe_secret_key
            .decrypt(cipher_text, &self.params)
    }
    ...
    // Decrypt with plain_modulus=2 and return message % 2
    #[inline]
    pub fn decrypt_2<M>(
        &self,
        cipher_text: &LweCiphertext<C>,
    ...
    where
        M: TryFrom<C> + std::ops::Rem<Output = M> + From<u8>,
    {
        let a_mul_s = cipher_modulus.reduce_dot_product(cipher_text.a(), &self.lwe_secret_key);
        let plaintext = cipher_modulus.reduce_sub(cipher_text.b(), a_mul_s);
        let t = 2u32;
