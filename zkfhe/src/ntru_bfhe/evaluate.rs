...
use fhe_core::{
    lwe_modulus_switch, KeySwitchingKey, LWECiphertext, LWEModulusType, NTRUBlindRotationKey,
    NTRUCiphertext, NTRUSecretKeyPack, Parameters,
};
use lattice::NTRU;
...
impl<F: NTTField> EvaluationKey<F> {
    ...
    pub fn new(secret_key_pack: NTRUSecretKeyPack<F>) -> Self {
        ...
        let key_switching_key = KeySwitchingKey::generate_ntru(&secret_key_pack, chi, &mut *csrng);
    ...
    pub fn bootstrap(&self, c: LWECiphertext, init_acc: NTRUCiphertext<F>) -> LWECiphertext {
        ...
        let key_switched = self.key_switching_key.key_switch_ntru(&acc);

        lwe_modulus_switch(key_switched, parameters.lwe_modulus().value())
    }
}
