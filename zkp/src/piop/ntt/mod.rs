...
/// Evaluation of NTT Instance at the same random point
// pub struct NTTInstanceEval<F: Field> {
//     /// store the coefficient representaions
//     pub coeffs: F,
//     /// store the point-evaluation representation
//     pub points: F,
...
// pub type NTTInstancesEval<F: Field> = Vec<F>;

/// Evaluations of NTT Instances at the same random point
// pub struct NTTInstancesEval<F: Field> {
//     /// store the evaluations of coefficient representaions
//     pub evals: Vec<F>,
// }
...
impl<F: Field> NTTInstance<F> {
    ...
    // #[inline]
    // pub fn evaluate(&self, point: &[F]) -> NTTInstanceEval<F> {
    //     NTTInstanceEval::<F> {
    //         coeffs: self.coeffs.evaluate(point),
    //         points: self.points.evaluate(point),
    //     }
    // }

    // Evaluate at a random point defined over Extension Field
    // #[inline]
    // pub fn evaluate_ext<EF: AbstractExtensionField<F>>(&self, point: &[EF]) -> NTTInstanceEval<EF> {
    //     NTTInstanceEval::<EF> {
    //         coeffs: self.coeffs.evaluate_ext(point),
    //         points: self.points.evaluate_ext(point),
    //     }
    // }
}
...
impl<F: Field> NTTInstances<F> {
    ...
    // Evaluate at a random point defined over Field
    // #[inline]
    // pub fn evaluate(&self, point: &[F]) -> NTTInstancesEval<F> {
    //     NTTInstancesEval::<F> {
    //         points: self.points.iter().map(|x| x.evaluate(point)).collect(),
    //         coeffs: self.coeffs.iter().map(|x| x.evaluate(point)).collect(),
    //     }
    // }
    ...
    // ) -> NTTInstancesEval<EF> {
    //     NTTInstancesEval::<EF> {
    //         points: self.points.iter().map(|x| x.evaluate_ext(point)).collect(),
    //         coeffs: self.coeffs.iter().map(|x| x.evaluate_ext(point)).collect(),
    //     }
    // }
}
...
impl<F: Field + Serialize> NTTIOP<F> {
    ...
    pub fn prove(
        ...
        let recursive_proof =
            <NTTIOP<F>>::prove_recursive(&mut trans, &state, &instance.info(), &u);
    ...
    pub fn verify(
        ...
        if !(subclaim.expected_evaluations == F::zero() && *claimed_sum == F::zero()){
            return false
        }
    ...
    //         NTTBareIOP::prove_as_subprotocol(randomness, f_u, claimed_sum, ntt_instance, u);
