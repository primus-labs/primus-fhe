...
use ntt_bare::{NTTBareProof, NTTBareSubclaim, NTTBareIOP};
...
pub fn init_fourier_table_overall<F: Field>(u: &[F], ntt_table: &[F]) -> (Vec<Rc<DenseMultilinearExtension<F>>>, Vec<Rc<DenseMultilinearExtension<F>>>) {
    ...
    // stores all the intermediate evaluations of the table (i.e. F(u, x)) in each iteration
    let mut rounds_evaluations = Vec::with_capacity(log_n);
    // stores the intermediate evaluations of the term ω^{2^{i + 1} * X} in each iteration
...
pub fn naive_w_power_times_x_table<F: Field>(ntt_table: &[F], log_m: usize, x_dim: usize, exp: usize) -> DenseMultilinearExtension<F> {
...
pub fn eval_w_power_times_x<F: Field>(ntt_table: &[F], log_m: usize, x_dim: usize, exp: usize, r: &[F]) -> F {
...
impl<F: Field> NTTSubclaim<F> {
    ...
    pub fn verify_subcliam(
        ...
        if !self.ntt_bare_subclaim.verify_subcliam_with_delegation(self.delegation_first_claim, points, coeffs, u, info) {
            return false;
        }

        // check2: check the final subclaim after delegating the computation of F(u, x) 
...
impl<F: Field> NTTIOP<F> {
    ...
    pub fn prove_as_subprotocol(
        ...
        let (evals_f, evals_w) = init_fourier_table_overall(&u, &ntt_instance.ntt_table);
        
        // 1. prove a(u) = \sum_{x\in \{0, 1\}^{\log N} c(x)\cdot F(u, x) } for a random point u
        let f_u = &evals_f[log_n - 1];
        let (ntt_bare_proof, state) = NTTBareIOP::prove_as_subprotocol(fs_rng, f_u, ntt_instance, &u);

        // the above sumcheck is reduced to prove F(u, v) where v is the requested point
        let mut requested_point = state.randomness;
        let mut reduced_claim = evals_f[log_n - 1].evaluate(&requested_point);
    ...
    pub fn delegation_verify_round(
        ...
        let eval = eval_identity_function(x_b_point, &r_left) * reduced_claim * (F::ONE - u_i + u_i * w_left) 
                    + eval_identity_function(x_b_point, &r_right) * reduced_claim * (F::ONE - u_i + u_i * w_right) * ntt_table[1 << round];
    ...
    pub fn verify_as_subprotocol(
        ...
        let ntt_bare_subclaim = NTTBareIOP::verify_as_subprotocol(fs_rng, &proof.ntt_bare_proof, ntt_instance_info);
        ...
        for (cnt, k, ) in (1..log_n).rev().enumerate() {
            ...
            let subclaim = MLSumcheck::verify_as_subprotocol(fs_rng, &poly_info,  proof.delegation_claimed_sums[cnt], &proof.delegation_sumcheck_msgs[cnt])
            ...
            // check the equality
            if !Self::delegation_verify_round(k, &requested_point, u[i], &subclaim, reduced_claim, ntt_instance_info){
...
mod test {
    ...
    fn test_init_fourier_table_overall() {
        ...
        let partial_fourier_mle = &init_fourier_table_overall(&u, &ntt_table).0[dim - 1];
