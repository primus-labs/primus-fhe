...
impl<F: Field + Serialize> NTTBareIOP<F> {
    ...
    // pub fn batch_sample_coins(trans: &mut Transcript<F>, instance: &NTTInstances<F>) -> Vec<F> {
    ...
    /// sample coins before proving sumcheck protocol
    // pub fn batch_sample_coins_ef<EF: AbstractExtensionField<F>>(trans: &mut Transcript<EF>, instance: &NTTInstances<F>) -> Vec<EF> {
    //     // batch `len_bits` sumcheck protocols into one with random linear combination
    //     trans.get_vec_challenge(
    //         b"randomness to combine sumcheck protocols",
    //         instance.num_ntt,
    ...
    // pub fn batch_num_coins(info: &NTTInstanceInfo<F>) -> usize {
    //     info.num_ntt
    // }
    ...
    pub fn prove(
        ...
        (proof, state, poly.info(), claimed_sum, evals_u)
    }
    ...
    pub fn verify(
        ...
        let mut subclaim =
            MLSumcheck::verify_as_subprotocol(&mut trans, &poly_info, *claimed_sum, &proof)
        ...
        subclaim.expected_evaluations == F::zero() && *claimed_sum == F::zero()
    }
    ...
    //     poly_info: &PolynomialInfo,
    //     claimed_sum: &mut F,
    //     evals_at_r: &NTTInstancesEval<F>,
    //     evals_at_u: &NTTInstancesEval<F>,
    //     info: &NTTInstanceInfo<F>,
    //     f_delegation: F,
    // ) -> bool {
    //     let mut trans = Transcript::new();
    ...
    //     if !Self::batch_verify_as_subprotocol(&randomness, &mut subclaim, &mut claimed_sum, evals_at_r, evals_at_u, info, f_delegation) {
    //         return false;
    //     }
    //     subclaim.expected_evaluations == F::zero() && claimed_sum == F::zero()
    // }

    // pub fn batch_verify_as_subprotocol(
    //     randomness: &[F],
    //     subclaim: &mut SubClaim<F>,
    //     claimed_sum: &mut F,
    //     evals_at_r: &NTTInstancesEval<F>,
    ...
    //     f_delegation: F,
    // ) -> bool {
    //     assert_eq!(randomness.len(), info.num_ntt);
    //     let eval_at_r = randomness.iter().zip(evals_at_r.coeffs.iter())
    //         .fold(F::zero(), |acc, (r, coeff)| acc + *r * *coeff);
    //     subclaim.expected_evaluations -= eval_at_r * f_delegation;
    //     let eval_at_u = randomness.iter().zip(evals_at_u.points.iter())
    //         .fold(F::zero(), |acc, (r, point)| acc + *r * *point);
    //     *claimed_sum -= eval_at_u;
    //     true
    // }

    // pub fn batch_prove(
    //     instance: &NTTInstances<F>,
    // )->(Proof<F>, ProverState<F>, PolynomialInfo, NTTInstance<F>)
    // {
    //     let mut trans = Transcript::<F>::new();
    //     let u = trans.get_vec_challenge(
    //         b"random point used to instantiate sumcheck protocol",
    ...
    //     let mut poly = ListOfProductsOfPolynomials::<F>::new(instance.num_vars);
    //     let mut claimed_sum = F::zero();
    //     let randomness = Self::batch_sample_coins(&mut trans, &instance);
    //     let random_instance = Self::batch_prove_as_subprotocol(&randomness, &mut poly, &mut claimed_sum, &instance, &u);

    //     let (proof, state) = MLSumcheck::prove_as_subprotocol(&mut trans, &poly)
    //         .expect("fail to prove the sumcheck protocol");
    //     (proof, state, poly.info(), random_instance)
    // }

    // pub fn batch_prove_as_subprotocol(
    //     randomness: &[F],
    //     poly: &mut ListOfProductsOfPolynomials<F>,
    //     claimed_sum: &mut F,
    //     instance: &NTTInstances<F>,
    //     u: &[F],
    // ) -> NTTInstance<F>{
    //     let random_instance = instance.to_basic::<F>(randomness);

    //     Self::prove_as_subprotocol(F::one(), poly, claimed_sum, &random_instance, u);
    //     // let f_u = init_fourier_table(u, &random_instance.ntt_table);
    //     // poly.add_product(
    //     //     [Rc::new(f_u), Rc::clone(&random_instance.coeffs)],
    //     //     F::one(),
    //     // );
    //     // *claimed_sum += random_instance.points.evaluate(u);
    //     random_instance
    // }

    // pub fn batch_prove_ef<EF: AbstractExtensionField<F>>(
    //     instance: &NTTInstances<F>,
    // )->(Proof<EF>, ProverState<EF>, PolynomialInfo, NTTInstance<EF>)
    // {
    ...
    //     let randomness = Self::batch_sample_coins_ef::<EF>(&mut trans, &instance);
    //     let random_instance = Self::batch_prove_as_subprotocol_ef::<EF>(&randomness, &mut poly, &mut claimed_sum, &instance, &u);
    ...
    // pub fn batch_prove_as_subprotocol_ef<EF: AbstractExtensionField<F>>(
    //     randomness: &[EF],
    ...
    //     u: &[EF],
    // ) -> NTTInstance<EF>{
    //     let random_instance = instance.to_ef::<EF>(randomness);
    //     <NTTBareIOP<EF>>::prove_as_subprotocol(randomness, poly, claimed_sum, &random_instance, u);
    ...
    //     let log_n = ntt_instance.num_vars;
