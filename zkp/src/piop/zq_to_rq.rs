...
use algebra::{
    utils::Transcript, AbstractExtensionField, AsFrom, DecomposableField,
    DenseMultilinearExtension, DenseMultilinearExtensionBase, Field, ListOfProductsOfPolynomials,
    MultilinearExtension, PolynomialInfo, SparsePolynomial, UF,
};

/// proof generated by prover
pub struct TransformZqtoRQProof<F: Field, EF: AbstractExtensionField<F>> {
...
pub struct TransformZqtoRQInstance<F: Field> {
    ...
    pub r: Rc<DenseMultilinearExtensionBase<F>>,
    /// introduced witness s
    pub s: Rc<DenseMultilinearExtensionBase<F>>,
    /// introduced witness to check the range of a, b, c
    pub r_bits: DecomposedBits<F>,
}

/// Instance of extension vesion
pub struct TransformZqtoRQInstanceExt<F: Field, EF: AbstractExtensionField<F>> {
    /// inputs a
    pub a: Rc<DenseMultilinearExtension<F, EF>>,
    /// introduced witness k
    pub k: Rc<DenseMultilinearExtension<F, EF>>,
    /// introduced witness r
    pub r: Rc<DenseMultilinearExtension<F, EF>>,
    /// introduced witness s
    pub s: Rc<DenseMultilinearExtension<F, EF>>,
}

impl<F: Field, EF: AbstractExtensionField<F>> TransformZqtoRQInstanceExt<F, EF> {
    /// Construct an instance over the extension field from the original instance defined over the basic field
    pub fn from_base(input_base: &TransformZqtoRQInstance<F>) -> Self {
        Self {
            a: Rc::new(DenseMultilinearExtension::from_base(&input_base.a)),
            k: Rc::new(DenseMultilinearExtension::from_base(&input_base.k)),
            r: Rc::new(DenseMultilinearExtension::from_base(&input_base.r)),
            s: Rc::new(DenseMultilinearExtension::from_base(&input_base.s)),
        }
    }
}
...
impl<F: Field + DecomposableField> TransformZqtoRQInstance<F> {
    ...
    pub fn from_vec(
        ...
        a: Rc<DenseMultilinearExtensionBase<F>>,
        k: &Rc<DenseMultilinearExtensionBase<F>>,
        r: &Rc<DenseMultilinearExtensionBase<F>>,
        s: &Rc<DenseMultilinearExtensionBase<F>>,
        base: F,
        base_len: u32,
        bits_len: u32,
    ) -> Self {
...
pub struct TransformZqtoRQ<F: Field, EF: AbstractExtensionField<F>>(
    PhantomData<F>,
    PhantomData<EF>,
);

impl<F: Field, EF: AbstractExtensionField<F>> TransformZqtoRQ<F, EF> {
    /// Prove transformation from a \in Zq to c \in R/QR
    pub fn prove(
        trans: &mut Transcript<F>,
        instance_base: &TransformZqtoRQInstance<F>,
        u: &[EF],
    ) -> TransformZqtoRQProof<F, EF> {
        let dim = u.len();
        assert_eq!(dim, instance_base.num_vars);

        // 1. rangecheck for r
        let rangecheck_msg = BitDecomposition::prove(trans, &instance_base.r_bits, u);

        // Convert the original instance defined over F to a new instance defined over EF
        let instance = <TransformZqtoRQInstanceExt<F, EF>>::from_base(instance_base);
        ...
        product.push(Rc::clone(&instance.k));
        op_coefficient.push((UF::one(), UF::zero()));
        product.push(Rc::clone(&instance.k));
        op_coefficient.push((UF::BaseField(-F::one()), UF::one()));
        poly.add_product_with_linear_op(product, &op_coefficient, EF::one());
        ...
        product.push(Rc::new(gen_identity_evaluations(u)));
        op_coefficient.push((UF::one(), UF::zero()));
        product.push(Rc::clone(&instance.r));
        op_coefficient.push((UF::one(), UF::one()));
        product.push(Rc::clone(&instance.k));
        op_coefficient.push((UF::BaseField(-(F::one() + F::one())), UF::one()));
        poly.add_product_with_linear_op(product, &op_coefficient, EF::one());
        ...
        product.push(Rc::new(gen_identity_evaluations(u)));
        op_coefficient.push((UF::one(), UF::zero()));
        product.push(Rc::clone(&instance.s));
        op_coefficient.push((UF::BaseField(-F::one()), UF::zero()));
        poly.add_product_with_linear_op(product, &op_coefficient, EF::one());
        ...
        let c_num_vars = instance_base.n.ilog(2) as usize;

        // construct c_u
        let eq_u = gen_identity_evaluations(u).evaluations;
        let mut c_u_evaluations = vec![EF::zero(); instance_base.n];
        instance_base
            .c
            .iter()
            .enumerate()
        ...
        // construct t
        let t_evaluations = (1..=instance_base.n)
            .map(|i| F::new(F::Value::as_from(i as f64)))
            .collect();
        let t = Rc::new(DenseMultilinearExtensionBase::from_evaluations_vec(
            c_num_vars,
            t_evaluations,
        ));

        // Convert to the MLE defined over the Extension Field
        let t = Rc::new(<DenseMultilinearExtension<F, EF>>::from_base(&t));

        let mut poly = <ListOfProductsOfPolynomials<F, EF>>::new(c_num_vars);
        let mut product = Vec::with_capacity(2);
        let mut op_coefficient = Vec::with_capacity(2);
    ...
    pub fn verify(
        trans: &mut Transcript<F>,
        proof: &TransformZqtoRQProof<F, EF>,
        decomposed_bits_info: &DecomposedBitsInfo<F>,
        c_num_vars: usize,
    ) -> TransformZqtoRQSubclaim<F, EF> {
        // 1. rangecheck
        ...
        let first_subclaim =
            MLSumcheck::verify(trans, &poly_info, EF::zero(), &proof.sumcheck_msgs[0])
                .expect("sumcheck protocol for transformation from Zq to RQ failed");

        let second_subclaim =
            MLSumcheck::verify(trans, &poly_info, EF::zero(), &proof.sumcheck_msgs[1])
                .expect("sumcheck protocol for transformation from Zq to RQ failed");
        ...
        let third_subclaim =
            MLSumcheck::verify(trans, &poly_info, proof.s_u, &proof.sumcheck_msgs[2])
                .expect("sumcheck protocol for transformation from Zq to RQ failed");
...
impl<F: Field, EF: AbstractExtensionField<F>> TransformZqtoRQSubclaim<F, EF> {
    /// verify the sumcliam
    ...
    pub fn verify_subclaim(
        ...
        q: usize,
        a: Rc<DenseMultilinearExtensionBase<F>>,
        c_dense: &DenseMultilinearExtensionBase<F>,
        k: &DenseMultilinearExtensionBase<F>,
        r: &[Rc<DenseMultilinearExtensionBase<F>>],
        s: &DenseMultilinearExtensionBase<F>,
        r_bits: &[&Vec<Rc<DenseMultilinearExtensionBase<F>>>],
        u: &[EF],
        ...
        let eval_k = k.evaluate_ext(&self.sumcheck_points[0]);
        if eval_identity_function(u, &self.sumcheck_points[0]) * eval_k * (EF::one() - eval_k)
        ...
        if eval_identity_function(u, &self.sumcheck_points[1])
            * ((r[0].evaluate_ext(&self.sumcheck_points[1]) + F::one())
                * (EF::one() - k.evaluate_ext(&self.sumcheck_points[1]) * (F::one() + F::one()))
                - s.evaluate_ext(&self.sumcheck_points[1]))
        ...
        let eval_c_u = c_dense.evaluate_ext(&[&self.sumcheck_points[2], u].concat());
        ...
        if eval_c_u * t.evaluate_ext(&self.sumcheck_points[2])
            != self.sumcheck_expected_evaluations[2]
        {
            return false;
        }
        ...
        a.evaluate_ext(u) * (F::one() + F::one()) * n_divied_by_q
            == k.evaluate_ext(u) * n + r[0].evaluate_ext(u)
    }
