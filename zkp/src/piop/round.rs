...
use algebra::{DenseMultilinearExtension, Field, ListOfProductsOfPolynomials, PolynomialInfo, UF};
...
/// Round IOP
pub struct RoundIOP<F: Field, EF: AbstractExtensionField<F>>(PhantomData<F>, PhantomData<EF>);

/// proof generated by prover
pub struct RoundIOPProof<F: Field, EF: AbstractExtensionField<F>> {
    /// range check proof for output b
    pub bit_decomp_proof_output: BitDecompositionProof<F, EF>,
    /// range check proof for offset c - 1
    pub bit_decomp_proof_offset: BitDecompositionProof<F, EF>,
    /// sumcheck msg
...
pub struct RoundInstance<F: Field> {
    ...
    pub input: Rc<DenseMultilinearExtensionBase<F>>,
    /// output denoted by b \in F_q
    pub output: Rc<DenseMultilinearExtensionBase<F>>,
    ...
    pub offset: Rc<DenseMultilinearExtensionBase<F>>,
    ...
    pub option: Rc<DenseMultilinearExtensionBase<F>>,
}

/// Store the corresponding MLE of RoundInstance where the evaluations are over the extension field.
pub struct RoundInstanceExt<F: Field, EF: AbstractExtensionField<F>> {
    /// input denoted by a \in F_Q
    pub input: Rc<DenseMultilinearExtension<F, EF>>,
    /// output denoted by b \in F_q
    pub output: Rc<DenseMultilinearExtension<F, EF>>,

    /// offset denoted by c = a - b * k \in [1, k] such that c - 1 \in [0, k)
    pub offset: Rc<DenseMultilinearExtension<F, EF>>,

    /// option denoted by w \in {0, 1}
    pub option: Rc<DenseMultilinearExtension<F, EF>>,
}

impl<F: Field, EF: AbstractExtensionField<F>> RoundInstanceExt<F, EF> {
    fn from_base(input_base: &RoundInstance<F>) -> Self {
        Self {
            input: Rc::new(<DenseMultilinearExtension<F, EF>>::from_base(
                input_base.input.as_ref(),
            )),
            output: Rc::new(<DenseMultilinearExtension<F, EF>>::from_base(
                input_base.output.as_ref(),
            )),
            offset: Rc::new(<DenseMultilinearExtension<F, EF>>::from_base(
                input_base.offset.as_ref(),
            )),
            option: Rc::new(<DenseMultilinearExtension<F, EF>>::from_base(
                input_base.option.as_ref(),
            )),
        }
    }
}
...
impl<F: DecomposableField> RoundInstance<F> {
    ...
    pub fn new(
        k: F,
        delta: F,
        input: Rc<DenseMultilinearExtensionBase<F>>,
        output: Rc<DenseMultilinearExtensionBase<F>>,
        ...
        // set w = 1 iff a = 0 & b = 0
        let option = Rc::new(DenseMultilinearExtensionBase::<F>::from_evaluations_vec(
            num_vars,
            input
                .iter()
                .zip(output.iter())
        ...
        let offset = Rc::new(DenseMultilinearExtensionBase::from_evaluations_vec(
            num_vars,
        ...
        // c - 1
        let c_minus_one = DenseMultilinearExtensionBase::from_evaluations_vec(
            num_vars,
            offset.iter().map(|x| *x - F::one()).collect(),
        );
        // c - 1 + delta
        let c_minus_one_delta = DenseMultilinearExtensionBase::from_evaluations_vec(
            num_vars,
...
impl<F: Field, EF: AbstractExtensionField<F>> RoundIOPSubclaim<F, EF> {
    /// verify the subclaim
    #[allow(clippy::too_many_arguments)]
    pub fn verify_subclaim(
        &self,
        u: &[EF],
        (lambda_1, lambda_2): (EF, EF),
        input: &Rc<DenseMultilinearExtensionBase<F>>,
        output: &Rc<DenseMultilinearExtensionBase<F>>,
        output_bits: &Vec<Rc<DenseMultilinearExtensionBase<F>>>,
        offset: &Rc<DenseMultilinearExtensionBase<F>>,
        offset_aux_bits_1: &Vec<Rc<DenseMultilinearExtensionBase<F>>>,
        offset_aux_bits_2: &Vec<Rc<DenseMultilinearExtensionBase<F>>>,
        option: &Rc<DenseMultilinearExtensionBase<F>>,
        ...
        let c_minus_one = Rc::new(DenseMultilinearExtensionBase::from_evaluations_vec(
            offset.num_vars,
            offset.iter().map(|x| *x - F::one()).collect(),
        ));
        let c_minus_one_delta = Rc::new(DenseMultilinearExtensionBase::from_evaluations_vec(
            offset.num_vars,
        ...
        let option_eval = option.evaluate_ext(&self.sumcheck_subclaim.point);
        let input_eval = input.evaluate_ext(&self.sumcheck_subclaim.point);
        let output_eval = output.evaluate_ext(&self.sumcheck_subclaim.point);
        let offset_eval = offset.evaluate_ext(&self.sumcheck_subclaim.point);

        self.sumcheck_subclaim.expected_evaluations
            == r_1 * eq_val * option_eval * (EF::one() - option_eval)
                + r_2
                    * eq_val
...
impl<F: Field, EF: AbstractExtensionField<F>> RoundIOP<F, EF> {
    ...
    pub fn prove(
        ...
        // Convert the original instance over Field to a new instance over Extension Field
        let instance_ext = <RoundInstanceExt<F, EF>>::from_base(instance);

        let mut poly = <ListOfProductsOfPolynomials<F, EF>>::new(instance.num_vars);
        let identity_func_at_u = Rc::new(gen_identity_evaluations(u));

        // randomly combine two sumcheck protocols
        let r = trans.get_vec_ext_field_challenge(b"randomness for sumcheck", 2);
        let r_1 = r[0];
        let r_2 = r[1];
        ...
        poly.add_product_with_linear_op(
            [
                ...
                Rc::clone(&instance_ext.option),
                Rc::clone(&instance_ext.option),
            ],
            &[
                (UF::one(), UF::zero()),
                (UF::one(), UF::zero()),
                (UF::BaseField(-F::one()), UF::one()),
            ],
            r_1,
        );
        ...
        poly.add_product_with_linear_op(
            [
                Rc::clone(&identity_func_at_u),
                Rc::clone(&instance_ext.option),
                Rc::clone(&instance_ext.input),
            ],
            &[
                (UF::one(), UF::zero()),
                (UF::one(), UF::zero()),
                (UF::ExtensionField(lambda_1), UF::zero()),
            ],
            r_2,
        );
        ...
        poly.add_product_with_linear_op(
            [
                Rc::clone(&identity_func_at_u),
                Rc::clone(&instance_ext.option),
                Rc::clone(&instance_ext.output),
            ],
            &[
                (UF::one(), UF::zero()),
                (UF::one(), UF::zero()),
                (UF::ExtensionField(lambda_2), UF::zero()),
            ],
            r_2,
        );
        ...
        poly.add_product_with_linear_op(
            ...
            &[
                (UF::one(), UF::zero()),
                (UF::BaseField(-F::one()), UF::one()),
                (UF::BaseField(-instance.k), UF::zero()),
            ],
            r_2,
        );
        ...
        poly.add_product_with_linear_op(
            [
                Rc::clone(&identity_func_at_u),
                Rc::clone(&instance_ext.option),
                Rc::clone(&instance_ext.offset),
            ],
            &[
                (UF::one(), UF::zero()),
                (UF::BaseField(-F::one()), UF::one()),
                (UF::BaseField(-F::one()), UF::zero()),
            ],
            r_2,
        );

        RoundIOPProof {
            bit_decomp_proof_output: BitDecomposition::prove(trans, &instance.output_bits, u),
            bit_decomp_proof_offset: BitDecomposition::prove(trans, &instance.offset_aux_bits, u),
    ...
    pub fn verify(
        trans: &mut Transcript<F>,
        proof: &RoundIOPProof<F, EF>,
        info: &RoundInstanceInfo<F>,
    ) -> RoundIOPSubclaim<F, EF> {
        ...
        RoundIOPSubclaim {
            bit_decomp_output_subclaim: BitDecomposition::verify(
                trans,
                &proof.bit_decomp_proof_output,
                &info.output_bits_info,
            ),
            bit_decomp_offset_subclaim: BitDecomposition::verify(
                trans,
