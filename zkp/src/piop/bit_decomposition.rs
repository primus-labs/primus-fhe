...
impl<F: Field> DecomposedBits<F> {
    ...
    pub fn pack_all_mles(&self) -> Vec<F> {
        assert_eq!(self.d_val.len(), self.d_bits.len());
    ...
    pub fn generate_oracle(
        ...
        assert_eq!(self.d_val.len(), self.d_bits.len());
        let num_oracles = self.d_val.len() + self.d_bits.len();
...
impl<F: Field + Serialize> BitDecomposition<F> {
    ...
    pub fn prove(instance: &DecomposedBits<F>) -> (Proof<F>, ProverState<F>, PolynomialInfo) {
        ...
        let u = trans.get_vec_challenge(b"random point used to instantiate sumcheck protocol", instance.num_vars);
    ...
    pub fn verify(
        ...
        let u = trans.get_vec_challenge(b"random point used to instantiate sumcheck protocol", info.num_vars);
    ...
    pub fn prove_sample_coins(trans: &mut Transcript<F>, instance: &DecomposedBits<F>) -> Vec<F> {
        // batch `len_bits` sumcheck protocols into one with random linear combination
        trans.get_vec_challenge(b"randomness to combine sumcheck protocols", instance.d_bits.len())
    }
    ...
    pub fn verify_sample_coins(trans: &mut Transcript<F>, info: &DecomposedBitsInfo<F>) -> Vec<F> {
        trans.get_vec_challenge(b"randomness to combine sumcheck protocols", info.bits_len as usize * info.num_instances)
    }
