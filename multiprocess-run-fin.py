...
def setup_tc_and_iptables(base_port, num_parties, bandwidth_mbps, delay_ms):
    print(f"🌐 设置 tc + iptables：限速 {bandwidth_mbps}Mbps，延迟 {delay_ms}ms")
...
def main():
    if len(sys.argv) != 5:
        print(
            "用法: python3 multiprocess-run-fin.py <NUM_PARTIES> <BASE_PORT> <BANDWIDTH_MBPS> <DELAY_MS>"
        )
        sys.exit(1)
