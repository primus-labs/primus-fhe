...
impl<F: NTTField> Parameters<F> {
    ...
    pub fn new(params: ConstParameters<F::Value>) -> Result<Self, FHECoreError> {
        ...
        // q|2N
        let lwe_modulus_u = params.lwe_modulus as usize;
        let twice_ring_dimension_div_lwe_modulus = (params.ring_dimension << 1) / lwe_modulus_u;
        if twice_ring_dimension_div_lwe_modulus * lwe_modulus_u != (params.ring_dimension << 1) {
            return Err(FHECoreError::LweModulusRlweDimensionNotCompatible {
                lwe_modulus: lwe_modulus_u,
                rlwe_dimension: params.ring_dimension,
            });
        }

        // 2N|(Q-1)
        let ring_modulus_u = cast::<<F as Field>::Value, usize>(params.ring_modulus).unwrap();
        let temp = (ring_modulus_u - 1) / (params.ring_dimension << 1);
        if temp * (params.ring_dimension << 1) != (ring_modulus_u - 1) {
            return Err(FHECoreError::RLweModulusRlweDimensionNotCompatible {
                rlwe_modulus: ring_modulus_u,
                rlwe_dimension: params.ring_dimension,
            });
        }
        ...
        let lwe_params = LWEParameters {
            ...
            lwe_modulus: PowOf2Modulus::<LWEModulusType>::new(params.lwe_modulus),
        ...
        let blind_rotation_params = BlindRotationParameters::<F> {
            ...
            ring_modulus: params.ring_modulus,
    ...
    pub fn lwe_noise_distribution(&self) -> DiscreteGaussian<LWEModulusType> {
        DiscreteGaussian::new(
            self.lwe_params.lwe_modulus.value(),
            0.0,
            self.lwe_params.lwe_noise_std_dev,
        )
        .unwrap()
